...
def findall(request): #メールアドレスで検索を行いJsonファイルでuser情報を表示する。
    data = list(Event.objects.all().values('id','classid','title','end','details','classid__classname'))
    json_str = json.dumps(data, ensure_ascii=False, indent=2)
    return HttpResponse(json_str)
...
def create(request): #メールアドレスで検索を行いJsonファイルでuser情報を表示する。
    ...
    event = Event(classid=classtemp,title=eventname,details=eventdetails,end=end)
    event.save()
